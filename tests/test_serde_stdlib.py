"""Round-trip tests for ``MsgpackSerializer`` and ``BytesSerializer``.

These tests are the executable contract for what ``serde/libs/stdlib.py``
claims to support.  Each parametrized case asserts that the module-level
:func:`misen.utils.serde.save` / :func:`misen.utils.serde.load` round-trip
a value with both type and value fidelity:

    type(loaded) is type(original) and loaded == original

The error cases assert that values the serializer cannot represent surface
as :class:`UnserializableTypeError` rather than crashing inside ``msgspec``.
"""
...
def test_primitive_roundtrip(roundtrip: Roundtrip, value: Any) -> None:
    ...
    _assert_strict_roundtrip(value, loaded)
...
def test_bytes_uses_bytes_serializer(roundtrip: Roundtrip) -> None:
    loaded = roundtrip(b"hello world")
    assert type(loaded) is bytes
    assert loaded == b"hello world"


def test_bytearray_uses_bytes_serializer(roundtrip: Roundtrip) -> None:
...
# ---------------------------------------------------------------------------
# Error cases — must raise UnserializableTypeError, not crash
# ---------------------------------------------------------------------------


def test_big_int_raises_clean_error(roundtrip: Roundtrip) -> None:
    with pytest.raises(UnserializableTypeError, match="int"):
        roundtrip(2**100)
