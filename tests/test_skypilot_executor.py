...
def fake_sky(monkeypatch: pytest.MonkeyPatch) -> Iterator[types.SimpleNamespace]:
    """Install a minimal stub `sky` module so the executor can import it."""
    ...
    def _launch(task: object, **kwargs: Any) -> tuple[int, object]:
        launches.append({"task": task, **kwargs})
        return (123, object())

    def _queue(cluster_name: str) -> list[dict[str, Any]]:
        return queue_state.get(cluster_name, [])
    ...
    sky.launch = _launch  # type: ignore[attr-defined]
    sky.queue = _queue  # type: ignore[attr-defined]
    ...
    yield types.SimpleNamespace(module=sky, launches=launches, queue_state=queue_state)
...
def test_skypilot_job_state_pending_until_future_resolves() -> None:
    ...
    job = SkyPilotJob(work_unit=work_unit, cluster_name="misen-test", workspace=workspace)
    ...
    # Future resolves with success: state then comes from sky.queue.
    future.set_result(7)
    # Without a fake sky.queue, importing sky inside _query_cluster_states
    # will fail with ImportError (caught and treated as 'unknown' via the
    # except block in bulk_state). Inject a stub directly here so we can
    # exercise the success branch.
    fake = types.ModuleType("sky")
    fake.queue = lambda _name: [{"job_id": 7, "status": "RUNNING"}]  # type: ignore[attr-defined]
    sys.modules["sky"] = fake
    try:
        assert job.state() == "running"

        # Terminal state caches.
        fake.queue = lambda _name: [{"job_id": 7, "status": "SUCCEEDED"}]  # type: ignore[attr-defined]
        assert job.state() == "done"
        # Even after queue forgets the cluster, the cached terminal state holds.
        fake.queue = lambda _name: []  # type: ignore[attr-defined]
        assert job.state() == "done"
    finally:
        sys.modules.pop("sky", None)
