"""Handlers for numpy dtypes, scalars, and arrays."""
...
if importlib.util.find_spec("numpy") is not None:
    ...
    class NumpyArrayHandler(CollectionHandler):
        """Hash numpy arrays by shape/dtype/content, including object arrays."""
        ...
        def elements(obj: Any) -> list[Any]:
            ...
            if array.dtype.hasobject:
                return [array.dtype.str, shape, array.reshape(-1).tolist()]

            contiguous = np.ascontiguousarray(array)
            payload_hash = incremental_hash(lambda sink: sink.write(contiguous.tobytes()))
            return [contiguous.dtype.str, shape, payload_hash]
