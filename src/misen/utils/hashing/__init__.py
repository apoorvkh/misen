...
_handler_registry: TypeDispatchRegistry[type[Handler]] = TypeDispatchRegistry(
    by_type_name={**stdlib_handlers_by_type, **optional_handlers_by_type},
    candidates=[*stdlib_handlers, *optional_handlers],
...
def get_handler_versions() -> dict[str, int]:
    ...
    for type_name, handler_cls in _handler_registry.by_type_name.items():
        versions[type_name] = handler_cls.version
    for handler_cls in _handler_registry.candidates:
