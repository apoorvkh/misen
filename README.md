...
- Set `env_cache = false` on the executor to opt out of sharing: each snapshot then keeps its environments in a store private to that snapshot (nothing reused across submissions, everything removed on cleanup).
